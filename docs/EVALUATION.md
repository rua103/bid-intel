...
Web 工作台的 `/evaluation/draft` 生成空白 gold 结构和独立 predictions，不再把预测字段预填成 gold。`/evaluation/run` 拒绝完全空白的 gold；内容与预测完全相同时默认要求重新核验（忽略行 ID、顺序及 `2`/`2.000` 等写法差异）。合法的完美匹配可通过勾选独立核验声明、发送 `allow_identical_gold=true` 评测；报告保留该声明。此保护用于防止误操作，不能代替标注人和复核人的过程记录。
