...
| P1-1 | 只有中标方被抽成投标主体 | 任务二 25 分 | 天 | 已修（待提交）：新增有明确表头和证据门控的投标/评审表抽取，候选/中标结果分开；未知排名不猜结果；准确率与无结构正文仍待 gold 验证 |
| P1-2 | 尚无已核验 gold set | 准确性 15 分无法自证 | 天 | 标注流程已就绪（cd68aad）；24 条需队友对照原文标注，完成前无准确率结论 |
| P1-3 | 六类提交物一件未开工 | 综合 10 分 + 入场券 | 周 | 部分完成（待提交）：系统设计、技术路线/数据质量报告及汇报/演示材料草稿已生成；准确性结果待 gold，决赛模板及最终录音待队伍补充 |
| P1-4 | 演示三件套全缺（一键启动/样例包/离线兜底） | 演示 5 分 | 天 | 已实现（待提交）：启动/停止脚本、公告+附件 ZIP 和离线虚构关系数据集；本机端到端启动/登录/查数据及关闭回归通过 |
| P1-5 | 无登录体系、无评审方操作说明 | 任务三 15 分 | 天 | 已实现（待提交）：单一评审账号、签名会话及受保护 API；随机本机凭据已生成；文档指导，需从第二台评审设备做网络验收 |
