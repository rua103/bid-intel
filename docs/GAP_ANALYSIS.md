...
| # | 问题 | 影响 | 成本 | 状态 |
|---|---|---|---|---|
| P0-1 | ✅ hybrid 把同一标的原样重复输出 | 准确性（精确率） | 小时 | 待修 |
| P0-2 | ✅ 规则抽取把章节标题/联系块当标的行 | 准确性（精确率） | 小时 | 待修 |
| P0-3 | ✅ 评测接口三点击刷出 100% | 答辩诚信 | 小时 | 待修 |
| P0-4 | 评审方远程访问被 CORS 锁死 | 任务三 15 分 + 演示 5 分 | 小时 | 待修 |
| P0-5 | 局域网下标注工作台按钮全抛异常 | 标注（gold 唯一入口） | 小时 | 待修 |
| P0-6 | 表头别名缺 CCGP 标准列名 | 提取率 10 分 | 小时 | 待修 |
| P0-7 | CCGP 头部键值表完全没读 | 覆盖率 + 提取率 | 小时 | 待修 |
...
| P1-6 | 没有清库/数据集隔离 | 任务二 25 分 | 小时 | 待修 |
| P1-7 | 附件 .doc/.xls 不支持、PDF 表格库没装 | 覆盖率 10 分 | 天 | 待修 |

---

## 一、任务一：抽取链路（40 分）

### 1.1 ✅ 规则抽取把章节标题和联系块当标的行输出 · blocker · 小时
...
### 1.2 ✅ hybrid 把同一标的原样重复一遍 · blocker · 小时
...
### 1.3 ✅ 评测接口三点击就能刷出 100% · blocker · 小时
...
### 1.4 表头别名缺 CCGP 标准列名 · blocker · 小时
...
### 1.5 `category` 映射到「品目号」序号列 · high · 小时
...
### 1.6 CCGP 公告头部的 2 列键值表完全没被读 · high · 小时
...
**影响**：覆盖率 10 分（原文和附件各 5 分），附件这一半现在只能拿印象分。
...
### 3.1 ✅ 评审方远程访问被 CORS 与硬编码 apiBase 双向锁死 · blocker · 小时
...
### 3.2 ✅ 局域网 http 下 `crypto.randomUUID` 不存在，标注工作台按钮全抛异常 · blocker · 小时
