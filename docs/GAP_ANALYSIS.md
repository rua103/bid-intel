...
| P1-1 | 只有中标方被抽成投标主体 | 任务二 25 分 | 天 | 已修（59587ee）：规则解析接入投标/评审表；24 条官方样本 HTML 回放中 22 条产出 158 条主体候选；候选准确性、附件覆盖与自由文本正文仍待 Gold 核验 |
| P1-2 | 尚无已核验 gold set | 准确性 15 分无法自证 | 天 | 标注流程已就绪（cd68aad）；24 条需队友对照原文标注，完成前无准确率结论 |
| P1-3 | 六类提交物一件未开工 | 综合 10 分 + 入场券 | 周 | 部分完成（e4c5114）：系统设计与技术路线/数据质量报告已有 DOCX/PDF 草稿；无 Gold 结果；PPT/视频按队伍要求暂缓，正式模板及录制仍待补 |
| P1-4 | 演示三件套全缺（一键启动/样例包/离线兜底） | 演示 5 分 | 天 | 已实现（792ab95）：启动/停止脚本、公告+附件 ZIP 和离线虚构关系数据集；本机端到端启动/登录/查数据及关闭回归通过；样例数据不得当作成绩 |
| P1-5 | 无登录体系、无评审方操作说明 | 任务三 15 分 | 天 | 已实现（0d7768a、764bc1e）：单一评审账号、签名会话及受保护 API；随机本机凭据已生成；文档指导，仍需从第二台评审设备做网络验收 |
...
| GAP 4.3 | 官方数据接入与首批检查 | 数据完整性与端到端可运行性 | 天 | 全量管线已实跑（cd68aad）：1038/1038 入库、0 失败；无 gold，附件边界与候选质量仍待核验，见全量报告 |
| GAP 1.16 | 官方附件扩展名错配、旧 DOC 超时与额外格式 | 附件覆盖率与吞吐 | 天 | 部分解决（cd68aad）：RAR/7z、大包落盘及 RapidOCR 已在全量运行；仍有不支持格式、PDF 解析错误与损坏成员，WPS/GBQ7 等需逐项核验 |
| GAP 1.17 | 正文/附件重复与需求模板候选混入 | 精确率与金额聚合 | 天 | 基础修复已回归（cd68aad）：候选 9→4，剔除 4 条需求记录、合并 1 组重复；识别 7 份空模板，1 组品牌冲突保留待核验；全量角色和别名规则仍待验证 |
| GAP 4.14 | 官方包内保存了下载错误响应 | 源数据完整性与评测口径 | 天 | 自动识别隔离已修（cd68aad）：已知异常 87/87 命中，错误路径保留在警告；61 条公告的有效附件及评测口径仍待数据方确认，未替换原件 |
| GAP 4.9 | 后台任务、进度与断点续跑 | 全量处理稳定性 | 天 | 基础链路已实跑（cd68aad）：1038 条任务完成；首轮 13 条进度写入失败经锁修复后续跑归零，仍需其他机器/故障场景压测 |
...
### 1.16 官方附件实际格式与扩展名不一致、真实 DOC 转换超时 · high · 天

**2026-09-26 当前状态**：内容签名识别与 DOC 缓存保持有效；受控后台展开已处理 32 个已知 RAR、2 个 7z，得到 239/13 个叶文件，RapidOCR 已实跑。仍有 3 条公告带不支持格式提示、6 条公告含 PDF 流解析错误、1 个 RAR 成员因 `BadRarFile` 无法读取；WPS/GBQ7 和复杂表格边界没有整体消除。不能据此声称附件全覆盖。DOC 转换证据如下。

**2026-09-25 修复状态（历史）**：按真实 ZIP/OOXML、OLE 流、PDF 和图片签名分派，保留原名。对 OLE 的 104 种唯一内容重新核验，覆盖 2234 个 DOC、1 个 XLS，其中 2172 个伪 DOCX 均识别为 DOC；不等于已转换全部 103 种 DOC。默认转换时限 120 秒，跨进程串行复用专属配置并按内容缓存。两个原超时 DOC 分别 41.151/23.767 秒成功，再次读取 0.251/0.041 秒。以下为首轮证据。

2026-09-25 官方材料盘点：2172 个 .docx 实际含 WordDocument OLE 流，涉及 212 条公告，当前 DOCX 分支会报 BadZipFile；无扩展名 PDF/图片被跳过，RAR/7z/WPS 尚未覆盖。6 条正文＋附件试跑中，2 个 DOC 在当前 60 秒配置下超时。3 个外层包声明展开大小超过 200 MiB，实际深层 ZIP 仍受层数限制。P1-7 合成格式回归成立，但不代表这些真实变体已处理。证据与范围见 [官方接入检查](OFFICIAL_INTAKE_REVIEW.md)。

### 1.17 正文/附件的同标的重复与需求模板候选混入 · high · 天

**修复后状态**：跨文件匹配采用数值比较、包号约束及唯一兼容关系，保留双方证据；不合并不同包、同文件不同来源行、品牌/规格/数量单位冲突。采购文件按叶文件名识别，明确需求表及空模板不进成交结果和模型上下文。6 条回归 9→4：4 条需求记录被排除，1 组正文/XLS 重复合并；品牌简称与全称冲突的另一组仍保留。XLS 已补规格说明、品目分类及明确包号/成交报价列；工作簿参考编码表仍可能占用模型上下文（1.13），通用模板与别名识别未宣称完成。以下为首轮证据。

实测 t20260509_26531865：正文与 XLS 同名、同价格标的分别以 1 / 1.0 数量和不同小数位保存，现有字符串去重未合并；t20260501_26507438 同时输出真实报价 PDF 和磋商文件需求表候选。P0-1 解决单文件规则/模型合并，不能代替跨文件实体、包号及材料角色对齐。需求模板是否属于最终标的须对照中标结果，不能直接把招标文件所有表格当成交清单。详见 [官方接入检查](OFFICIAL_INTAKE_REVIEW.md)。
...
### 3.7 同步导入请求无超时、取消与进度反馈 · medium · 小时

**当前状态**：新增后台批量任务有逐公告进度、暂停、续跑和失败重试；下面的证据与影响仍适用于既有同步导入端点，不适用于后台 job 页面。

**证据**：同步导入路径仍无 `AbortController`/`signal`/`EventSource`/`onUploadProgress`。已知模型单条 24-90 秒，一条带 3 附件的公告会串行多轮，期间界面只有按钮文字变化（`正在解析…`）。
...
### 4.3 官方基准数据已完成全量接入，质量和口径仍待核验 · high · 天

**2026-09-26 当前状态**：独立数据集完成 1038/1038 公告端到端导入，0 条最终失败；共核验 2008 个入口文件的 SHA-256，无变化。任务采用 rules + 本地 RapidOCR，产生 6814 条标的候选，但投标参与和中标记录为 0。全量没有 gold、没有真实模型调用，不能宣称抽取准确率或评测通过。87 个无效下载附件、少量不支持格式、PDF 流错误和损坏成员仍需人工/数据方处理。运行证据见 [官方接入检查](OFFICIAL_INTAKE_REVIEW.md) 与 [机器摘要](benchmarks/official-full-20260926.json)。

**2026-09-25 状态（历史）**：官方材料已到，只有递归文件头盘点、正文规则基线及 6 条附件试跑；当时无完整全量入库。以下仍保留到货前开发集背景。

赛题八：基准数据集（约 1000 条，含附件）由命题方统一发放。到官方数据之前，开发语料是自采的 20 条 ccgp **纯 HTML、零附件**，因此"附件归属""附件型公告""扫描件"这些官方数据的常态形态在开发集里**毫无代表性**。

> ⚠️ **不要以为「官方数据到了只是换语料，管线一行不用改」。** 这次全量运行前实际补了格式识别、RAR/7z、落盘预算、OCR、任务检查点与恢复。处理完成只说明管线有产出；真实源文件仍有无法恢复或无法解析的部分，候选准确性仍要用独立 gold 验证。
...
另：默认 OCR 引擎为本地 RapidOCR，可通过 `.[ocr]` 安装，不依赖 Tesseract。官方全量运行产生 2261 条 OCR 识别/复核提示，说明识别路径已实跑；这不证明每份 OCR 文本正确，仍须抽样人工核验。若改用 Tesseract，才需要系统程序和中文语言包。
...
### 4.9 后台批量处理已实现并完成全量实跑，故障与并发边界仍需验证 · high · 天

**2026-09-26 当前状态**：持久任务、逐公告检查点、文件哈希检查、幂等入库、暂停/续跑和失败重试已实现；1038 条官方数据完成首轮处理及 21 条恢复/重算，最终 1038 完成、0 失败。首轮暴露 13 条 Windows 进度文件写入竞争；共享锁修复后实际续跑通过。全量使用 3 个本机进程，活跃墙钟按检查点估算约 66 分 45 秒；不能代表多用户并发、另一台机器或模型模式性能。后续仍需做不同磁盘/服务重启、多个任务竞争和实际 API/UI 恢复的压力测试，完整证据见 [官方接入检查](OFFICIAL_INTAKE_REVIEW.md)。

以下是实现前的历史证据：`storage.py` 当时只有 `PRAGMA foreign_keys=ON`，导入走分钟级同步请求，没有后台作业、进度端点或断点续跑；相关旧耗时不再代表当前批处理。
...
### 4.14 官方附件中含下载错误响应 · high · 数据方确认

**修复后状态**：gzip 解封装后识别验证码/维护/访问限制/JSON 错误；已知 87 份异常原件逐个回放，87/87 有明确不可用原因，不送模型。全量运行后数据集仍记录这 87 个不可用成员、分布在 61 条公告。单独上传全部无效材料会返回具体原因。只完成自动隔离，不能恢复本就缺失的正文或字段；仍需数据方提供有效附件或确认评测方式。首次发现记录如下。

首轮递归检查识别至少 87 个伪附件，涉及 61 条公告：48 个验证码页、31 个维护页、2 个访问限制页、6 个 JSON 错误（含 gzip 外包）。这些没有真实报价/声明内容，不能通过调整抽取模型恢复。完整成员清单已保存在本地 official-intake-20260925/source-file-issues.md 和 JSON；未联系数据提供方、未重新下载替换。该清单是 2026-09-25 有限盘点的历史范围，不代表其他压缩成员已验证可用。
...
- ✅ README 将 OCR 说明为可配置能力；当前默认 RapidOCR，Tesseract 仅作为可选引擎
