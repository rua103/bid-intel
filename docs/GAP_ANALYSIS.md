...
| P1-2 | 尚无已核验 gold set | 准确性 15 分无法自证 | 天 | 分层抽样、独立试标与标注工作台已就绪；24 条需队友对照原文标注，完成前无准确率结论 |
...
| GAP 4.3 | 官方数据接入与首批检查 | 数据完整性与端到端可运行性 | 天 | 全量管线已实跑：1038/1038 入库、0 失败；无 gold，附件边界与候选质量仍待核验，见全量报告 |
| GAP 1.16 | 官方附件扩展名错配、旧 DOC 超时与额外格式 | 附件覆盖率与吞吐 | 天 | 部分解决：RAR/7z、大包落盘及 RapidOCR 已在全量运行；仍有不支持格式、PDF 解析错误与损坏成员，WPS/GBQ7 等需逐项核验 |
| GAP 1.17 | 正文/附件重复与需求模板候选混入 | 精确率与金额聚合 | 天 | 基础修复已回归：候选 9→4，剔除 4 条需求记录、合并 1 组重复；识别 7 份空模板，1 组品牌冲突保留待核验；全量角色和别名规则仍待验证 |
| GAP 4.14 | 官方包内保存了下载错误响应 | 源数据完整性与评测口径 | 天 | 自动识别隔离已修：已知异常 87/87 命中，错误路径保留在警告；61 条公告的有效附件及评测口径仍待数据方确认，未替换原件 |
| GAP 4.9 | 后台任务、进度与断点续跑 | 全量处理稳定性 | 天 | 基础链路已实跑：1038 条任务完成；首轮 13 条进度写入失败经锁修复后续跑归零，仍需其他机器/故障场景压测 |
